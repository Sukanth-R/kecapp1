...
import "./mechhigh.css"; // Ensure this file is in the same folder
import MECHANICAL from './mech';

const MECHDepartmentHighlights = () => {
  ...
  useEffect(() => {
    const fetchHighlights = async () => {
      try {
        const response = await fetch("http://localhost:4001/api/highlights");
  ...
  useEffect(() => {
    const fetchMilestones = async () => {
      try {
        const response = await fetch("http://localhost:4001/api/milestones");
        const data = await response.json();
...
export default MECHDepartmentHighlights;
